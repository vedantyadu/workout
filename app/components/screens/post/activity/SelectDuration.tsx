...
import { DurationType } from '@/types/post'
...
export default function SelectDuration({
  ...
  value: DurationType
  setValue: (value: DurationType) => void
  ...
  const adjustHours = (delta: number) => {
    const current = parseInt(value.hours || '0')
    const next = Math.max(0, current + delta)
    updateHours(next.toString())
  }

  const adjustMinutes = (delta: number) => {
    const current = parseInt(value.minutes || '0')
          ...
          <View className='gap-2'>
            ...
            <View className='flex-row items-center justify-between'>
              ...
              <View className='flex-row items-baseline gap-2'>
                ...
                />
                <OutfitText className='text-neutral-400 text-lg'>
                  hrs
                </OutfitText>
              </View>

...
export default function PostTypeTouchable({ children, className, icon, title, description, ...props }: React.PropsWithChildren<TouchableOpacityProps> & { icon: React.ReactNode, title: string, description: string }) {
  return (
    <TouchableOpacity className="py-6 px-4 rounded-lg bg-neutral-100 flex-row items-center" {...props}>
      <View className="flex-1 flex-row items-center gap-4">
        <View className="size-12 justify-center items-center">
          {icon}
        </View>
        <View>
          <OutfitText weight="semi-bold" className="text-lg">{title}</OutfitText>
          <OutfitText className="text-sm !text-neutral-400">{description}</OutfitText>
        </View>
      </View>
